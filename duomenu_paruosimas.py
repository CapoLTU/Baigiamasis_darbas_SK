...
#_______________Pasitikrinam ar viskas veiks kaip reikia
def train_val_test(duom_dir, tikslas):
    # Gauname originalų DataFrame
    df_original = read_data(duom_dir)
    df = df_original.copy()
    ...
    # Pasiruošiam X ir y
    feature_pv = tikslas
    X = df.drop(columns=[feature_pv])
    y = df[[feature_pv]]

    # NormalizuojamTIK X reikšmėms
    scaler = MinMaxScaler()
    X_scaled = scaler.fit_transform(X)

    # Issisaugom scalerio param vėlesniam naudojimui
    joblib.dump(scaler, "scaler_X.pkl")

    # Pakeičiame `df` X dalį į normalizuotą
    ...
    # Iš naujo pasiimam jau normalizuotą `df`
